...
2. **模拟 Host + 真实 USB Coprocessor 模式（Real Device 页面）**

   ```text
   Host Sim (macOS, --scenario managed)  ←→  Sim Manager (Slint + Rust, macOS)  ←→  USB Coprocessor (真实硬件)
   ```

   Sim Manager 作为 USB Host 侧代理：它直接打开 USB 设备（默认 VID:PID `303A:8201`，界面 0，bulk OUT `0x01` / IN `0x81`），将 Host Sim 发来的 `WIRE_FRAME` payload 逐字节写入 USB，并把 USB 字节流重组出的标准 Frame 包回 `WIRE_FRAME` 记录发给 Host Sim。Session/Credit/Sequence/Heartbeat 仍由 Host Core 与固件负责，Manager 只是字节管道。

   页面提供 Wi-Fi 操作（Scan、Connect、Disconnect、Start AP、Stop AP），编码为 sideband `WIFI_COMMAND` 记录下发给 Host Sim 执行；命令结果与 Wi-Fi 事件（扫描结果、STA 连接/断开、AP client 加入/离开）由 Manager 在转发路径上 tap 解码并展示。USB 断开时 Manager 周期重试打开，链路恢复后由 Host Core 经 heartbeat 超时自行重新 Hello。

   Host Sim 也保留不经 Manager 的 `--usb` 独立直连模式，两种拓扑互不干扰。此模式用于在真实 Coprocessor 上复用同一套 Host Sim 调试能力。

#### 22.3.3 模拟器 IPC 监控扩展

标准 WL-hosted Frame 与模拟器控制消息共用同一 Unix socket，但两者通过外层 IPC Record 区分。IPC sideband 仅在 macOS 模拟器与 Sim Manager 之间使用，真实固件不启用。
...
magic[8]        = "WLHSIM\0\0"
version:u16_le  = 1
role:u8         = 1 HOST_SIM, 2 COPROC_SIM, 3 MANAGER
flags:u8        = bit 0 表示支持 sideband 扩展
max_record_size:u32_le
```

Hello 只在建立 Unix socket 连接时发送一次，用于声明本端角色与 sideband 能力，不参与 WL-hosted Link Hello Channel 的 channels 协商。

**IPC Record**

```text
record_len:u32_le   # 从 kind 字段到 payload 结束的字节数，即 4 + payload_len
kind:u8
flags:u8
reserved:u16
payload[...]
...
- `WIRE_FRAME` 的 payload 是标准 WL-hosted Frame。Manager 对其只剥离和重建 IPC 外层；未注入故障时，WL-hosted Frame 必须逐字节不变。
- `RUNTIME_INFO`、`FAULT_REQUEST`、`FAULT_RESPONSE` 保留原有职责（周期性上报模拟器内部状态、请求/响应故障注入），但改为由 IPC sideband protobuf 承载，不参与 WL-hosted Link Hello Channel 协商，也不影响标准 Frame 的 Session/Credit/Sequence 行为。
- `WIFI_COMMAND` 仅由 Manager 发往 Host Sim（Real Device 页面），请求执行扫描、连接、断开、开启/关闭 SoftAP 等标准 Wi-Fi 操作；命令结果与事件不经 sideband 回传，而是作为标准 Frame 出现在 `WIRE_FRAME` 流中，由 Manager tap 观察。

启用条件：

- sideband 仅在对端角色为 `MANAGER` 且本端与对方 `flags` 均声明支持 sideband 时才启用。
- Host/Coproc 直接对连（不经 Manager）时，sideband 静默禁用，两端仅使用标准 Wire Protocol。
- 真实 USB 模式下，Host Sim 仍可通过 sideband 向 Manager 上报运行状态；真实 Coprocessor 只使用标准 Wire Protocol，不需要实现模拟器扩展。

故障注入路径：

- 转发路径故障（丢帧、重复、乱序、延迟、篡改 checksum、截断 payload 等）由 Manager 在本地执行，作用对象是 `WIRE_FRAME` 的转发流程。
- Host/Coproc 内部故障通过 `FAULT_REQUEST` 下发，对端以 `FAULT_RESPONSE` 返回结果。
